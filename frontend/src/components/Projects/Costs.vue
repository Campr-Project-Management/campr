...
export default {
    methods: {
        ...mapActions(['getProjectCostsGraphData']),
    ...
    watch: {
        costData(value) {
            ...
            Object.entries(this.costData.byDepartment).map(([key, value]) => {
    ...
    data: function() {
        ...
        optionsByPhase.title = Translator.trans('message.costs_chart_by_phase');
        optionsByDepartment.title = Translator.trans('message.costs_chart_by_department');
        return {
            columns: [{
                'type': 'string',
