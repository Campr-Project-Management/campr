...
const actions = {
    ...
    /**
     * Remove attachment to an existing task
     * @param {function} commit
     * @param {array} data
     * @return {Object}
     */
    removeAttachmentTask({commit}, data) {
        return Vue.http.delete(
            Routing.generate('app_api_workpackage_attachment_delete',
                {'id': data.taskId, 'mediaId': data.mediaId}),
            data.data,
        ).then(
            (response) => {
                if (response.body && response.body.error) {
